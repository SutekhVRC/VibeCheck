...
#[cfg(target_os = "windows")]
pub const VERSION: &str = "0.1.8-beta-windows";

#[cfg(target_os = "linux")]
pub const VERSION: &str = "0.1.8-beta-linux";

pub struct VibeCheckVersion {
...
pub struct VibeCheckUpdater {
    pub current_version: Option<VibeCheckVersion>,
    pub newest_version: Option<VibeCheckVersion>,
    pub up_to_date: bool,
    pub release_blob: Option<Value>,
}

impl VibeCheckUpdater {
    pub fn new() -> Self {
        let mut vu = Self {
        ...
        vu.current_version = Self::parse_vibecheck_version_from_str(VERSION);
    ...
    #[cfg(target_os = "windows")]
    pub fn update_vibecheck(blob: Value) {
        let assets: &Vec<Value> = blob.as_array().unwrap()[0]
            .get("assets")
            .unwrap()
            .as_array()
    ...
    fn update_newest_release(&mut self) -> i8 {
        let http_cli = reqwest::blocking::Client::builder()
            ...
            .unwrap();
        match http_cli
            .get("https://api.github.com/repos/SutekhVRC/VibeCheck/releases?per_page=1")
            .send()
        {
            Ok(res) => {
                if res.status() == StatusCode::OK {
                    ...
                    let json_val_str = release_response.as_array().unwrap()[0]
                        ...
                        .as_str()
                        .unwrap();
                    match Self::parse_vibecheck_version_from_str(json_val_str) {
                        Some(new) => {
    ...
    fn parse_vibecheck_version_from_str(version: &str) -> Option<VibeCheckVersion> {
        ...
        let mut version_parse = VibeCheckVersion {
